...
// Frontier
pub use fc_rpc::{
	pending::ConsensusDataProvider as PendingConsensusDataProvider, EthBlockDataCacheTask, EthConfig,
};
...
pub struct EthDeps<B: BlockT, C, P, CT, CIDP> {
	...
	/// Consensus data provider used when building pending blocks.
	///
	/// `Some(AuraConsensusDataProvider::new(client))` on the Aura path;
	/// `None` on the Babe path (the digest is omitted for pending blocks).
	pub pending_consensus_data_provider: Option<Box<dyn PendingConsensusDataProvider<B>>>,
}
	...
	EC: EthConfig<B, C>,
{
	use fc_rpc::{
		...
		EthPubSub, EthPubSubApiServer, EthSigner, LogsJournal, Net, NetApiServer, Web3,
		Web3ApiServer,
	};
	#[cfg(feature = "txpool")]
