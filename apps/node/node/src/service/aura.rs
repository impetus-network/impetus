//! Aura + GRANDPA consensus service wiring for impulse (testnet) and dev chains.
//!
//! Contains the Aura import queue builder, manual-seal import queue builder,
//! the `new_full` service entry-point for Aura chains, and manual-seal authorship.
...
use crate::{
	cli::Sealing,
	client::{FullClient, RuntimeApiCollection},
	eth::{
		new_frontier_partial, spawn_frontier_tasks, EthConfiguration, FrontierBlockImport,
		FrontierPartialComponents,
	},
};

use super::common::{
	new_partial, FullSelectChain, GrandpaBlockImport, GRANDPA_JUSTIFICATION_PERIOD,
};
	...
	let rpc_builder = {
		...
		Box::new(move |subscription_task_executor| {
			let eth_deps = crate::rpc::EthDeps {
				...
				pending_consensus_data_provider: Some(Box::new(
	...
	HF: HostFunctionsT + 'static,
{
	...
	impl sp_inherents::InherentDataProvider for MockTimestampInherentDataProvider {
		async fn provide_inherent_data(
			...
			TIMESTAMP.with(|x| {
				*x.borrow_mut() += runtime_common::SLOT_DURATION;
